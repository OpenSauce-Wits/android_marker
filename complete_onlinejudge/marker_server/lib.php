 ...
 */
function build_project( $project_files)
{
	$zip = new ZipArchive() ;
	$success = true ;
	$message = "" ;
	...
	//unzip files
	//TODO check if testcases already exist before extracting
	$testcases = $zip->open( get_value( 'testcases', $project_files)) ;
	if( $testcases == TRUE)
	{
		$zip->extractTo( settings::$project_dir) ;
		$zip->close() ;
		log_( "Extracted ".get_value( 'testcase', $project_files)." to destination ".settings::$project_dir) ;
	}
	else
	{
		$success = false ;
		$message = "Failed to open ".get_value( 'testcases', $project_files) ;
		log_( $message) ;
	}

	//unzipping student's source files
	$source = $zip->open( get_value( 'source', $project_files)) ;
	if( $source == TRUE)
	{
		$zip->extractTo( settings::$source) ;
		$zip->close() ;
		log_( "Extracted ".get_value( 'source', $project_files)." to destination ".settings::$project_dir) ;
		$success = true ;
	}
	else
	{
		$success = false ;
		$message = "Failed to open ".get_value( 'source', $project_files) ;
		log_( $message) ;
	}
	
	/*//copy named source structure to .json
	$source_structure = copy( settings::$source_structure."/".$project_files['source_structure'], settings::$source_structure."/source_structure.json") ;
	if( !$source_structure)
	{
		$success = false ;
		$message = "Could not copy ".settings::$source_structure."/".$project_files['source_structure']." to a json file" ;
		log_( $message) ;
	}
	else
	{
	*/
	//read in json file
	$json_file_data = json_decode( file_get_contents( get_value( 'source_structure', $project_files))) ;
	$moving_source = move_source_files( $json_file_data) ;
	if( !($success = $moving_source[0]))
	{
		$message = $moving_source[1] ;
	}
	return array( $success => $success ? settings::$project_dir : $message) ;
}


/*Given an array @param $array and a key @param $key return the value corresponding to that key in the array
 */
function get_value( $key , $array)
{
	foreach ( $array as $arr)
	{
		foreach( $arr as $key_ => $value)
		{
			if( $key == $key_)
			{
				return $value ;
...
/*Given the root directory @param $project_dir of an android project, build an apk and @return { success : $message}
 */
/*function build_apk( $project_dir)
{
	$success = true ;
	$message = "" ;
...
*/
/* @param $json_file_data json file specifying source files from <tmp_marker>/source and their destination at <tmp_marker>/android_project
 * @return array( $success, $any_error_messages)
 */
function move_source_files( $json_file_data)
{
	$success = true ;
	$message = "" ;
	//TODO check student's dir against source structure
	foreach( $json_file_data as $file)
	{
		foreach( $file as $filename => $dirs)
		{
			$source_dir = $dirs[0] ;
			$dest_dir = $dirs[1] ;
			//copy file to destination inside project
			if( !file_exists( settings::$source."/".$source_dir))
			{
				$success = false ;
				$message = "File ".$source_dir." is missing." ;
			}
			else if( !file_exists( settings::$project_dir."/".$dest_dir))
			{
				$success = false ;
				$message = "File ".$dest_dir." is missing." ;
			}
			else if( !recurse_copy( settings::$source."/".$source_dir, settings::$project_dir."/".$dest_dir."/".$filename))
			{
				log_( settings::$source."/".$source_dir) ;
				log_( settings::$project_dir."/".$dest_dir) ;
				$success = false ;
				$message = "Failed to copy ".$filename." from ".$source_dir." to ".$dest_dir ;
				log_( $message) ;
				break ;
			}
		}

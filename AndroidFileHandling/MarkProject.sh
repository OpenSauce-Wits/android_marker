...
clear_files () {
  for file in $(ls)
  do
    if [ "$file" != "RequiredDocuments.txt" ] && [ "$file" != "runTestOnEmulator.sh" ] && [ "$file" != "LecturerZip.zip" ] && [ "$file" != "StudentZip.zip" ] && [ "$file" != "MarkProject.sh" ] && [ "$file" != "SetupOfAndroidEmulator.txt" ];
    then
      rm -rf "$file"
    fi
  done
...
open_project () {
  for file in $(ls)
  do
    if [ "$file" != "RequiredDocuments.txt" ] && [ "$file" != "runTestOnEmulator.sh" ] && [ "$file" != "PrepareTemplate.sh" ] && [ "$file" != "LecturerZip.zip" ] && [ "$file" != "StudentZip.zip" ] && [ "$file" != "MarkProject.sh" ] && [ "$file" != "SetupOfAndroidEmulator.txt" ];
    then
    	androidProject="$file"
      cd "$file"
    fi
